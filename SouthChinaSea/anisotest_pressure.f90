       ...
       do i=1-mbc,mx+mbc
          ...
          do jloc = 1, 3
             ...
             do iloc = 1, 3
                ...
                overPressure_y = computedOverPressure(dist_in_km,time,1.d0)/100. !  / 100 from percent to value
                !overPressure = computedOverPressure(dist_in_km,time)       !  this version returns dp

                theta = atan2(ydist,xdist)
                theta = theta + pi/4.d0 ! to make ellipse at 45 degree
                ! this makes it come in from the South East (at 45 deg below a latitude line)
...
double precision function  computedOverPressure(dist_in_km,time,scale)
   ...
   !real(kind=8),parameter :: ampl = 5., width = 5., tstar = .5;
    ...
    if ( dist_in_km <= t) then
       p_t  =  thick*2.d0
       c    =  width/2.35482d0
       g    =  maxAmp * exp(-(rad/c)**2)  ! /* ...Gaussian envelope */
       computedOverPressure = .5*g*exp(-.8d0*(t-rad)/p_t)*(1.0d0 - 1.1d0*(t-rad)/p_t)
    else
