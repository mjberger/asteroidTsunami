...
subroutine flag2refine2(mx,my,mbc,mbuff,meqn,maux,xlower,ylower,dx,dy,t,level, &
    ...
    logical :: allowflag, depth_forcing,wasted,allowed
    external allowflag
    logical :: use_topofiles
    
    ! Generic locals
    integer :: i,j,m, numFlagged
    ...
    pressure_refine_sq = 5.00D0**2  !since compared with square below
    maxGradP2 = 0.d0
    maxEta    = 0.d0
    depthTol = -1000.d0 
    depth_forcing = .true.
...
!           enddo
            ...
            ! Refine based on shallow depth  - but include wave tolerance so not
            ! all shallow water refined
            if (depth_forcing) then
                depth = aux(1,i,j)
                eta = q(1,i,j) + aux(1,i,j)

                if (depth .gt. depthTol .and. depth .lt. 0.d0 .and.           &
                    abs(eta - sea_level) > wave_tolerance     .and.           &
                    q(1,i,j) < deep_depth .and. q(1,i,j) > dry_tolerance) then
                     amrflags(i,j) = DOFLAG
                     cycle x_loop
                endif
            endif

           ! Refine based on regular wave tolerance
             eta = q(1,i,j) + aux(1,i,j)
             allowed = (allowflag(x_c,y_c,t,level)) ! this checks regions
             if (abs(eta-sea_level) > wave_tolerance .and. q(1,i,j) > dry_tolerance .and. allowed) then
                     amrflags(i,j) = DOFLAG
                     cycle x_loop
             endif
...
!    do j = 1, my
!        do i = 1, mx
!           if (amrflags(i,j) .eq. DOFLAG) then
!              numFlagged = numFlagged + 1
!           endif
!        end do
!    end do

!     write(*,101) xlower,ylower,numFlagged
101  format("on grid with low corner ",2e15.7," there are ",i5," flagged points")
