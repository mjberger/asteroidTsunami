...
%  if (i==2 || i>6) continue; end
  ...
  figure(figno);plot(time,eta); %plot(time,eta,col);
  hold on
  %figure(figno+1);plot(time,dp); %plot(time,dp,col);
  %hold on
  figure(figno+2);plot(time,q); %plot(time,q,col);
  hold on  
