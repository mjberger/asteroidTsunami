...
!     aux(pressure_index, i , j) = Pressure Field
    ...
    ! my original blast loc - deep water
    !blasty_center = 17.45
    !blastx_center = 117.38

    blasty_center = 13.33
    blastx_center = 115.26
...
double precision function  computedOverPressure(dist_in_km,time)
   ...
   double precision :: maxAmp1, maxAmp2, maxAmp3
   ...
   !real(kind=8),parameter :: ampl = 5., width = 5., tstar = .5;
    ...
    maxAmp1  = 8000.d0 !from MJA sims  ! *10 of Tunguska sized object 
    maxAmp2  =  200.d0 !from MJA sims  ! *10 of Tunguska sized object 
    maxAmp3  =  200.d0 !from MJA sims  ! *10 of Tunguska sized object 
    width   = 40.d0
    thick   = 5.d0 
    speed   = 0.3915d0
    ...
    if ( dist_in_km <= t) then
       p_t  =  thick*2.d0
       c    =  width/2.35482d0
       g    =  0 + maxAmp1 * exp(-4.50*(rad/c)**2)  ! /* ...Gaussian envelope */
       g    =  g + maxAmp2 * exp(-2.d0*(rad/(5.5*c))**2)  ! /* ...Gaussian envelope */
       g    =  g + maxAmp3 * exp(-2.d0*(rad/(3.*c))**4)  ! /* ...Gaussian envelope */
       !tail = 100.d0/(1.d0+(.05*rad)**2)
       !amplitude = g + tail
       !computedOverPressure = amplitude*exp(-5.d0*(t-rad)/p_t)*(1.0d0 - 5.0d0*(t-rad)/p_t)
       computedOverPressure = .5*g*exp(-3.8d0*(t-rad)/p_t)*(2.0d0 - 1.20d0*(t-rad)/p_t)
       if (computedOverPressure .lt. -50.d0) computedOverPressure = -50d0
    else
