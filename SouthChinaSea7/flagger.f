...
c
      ...
      real*8 maxFlag, maxFlag_thisGrid
...
!$OMP&            PRIVATE(locuse,maxFlag_thisGrid),
...
c no longer getting locbig, using "real" solution array in locnew
     ...
     &                        goodpt,badpt,maxFlag_thisGrid)

             maxFlag = max(maxFlag, maxFlag_thisGrid)
             endif     
c     
         if (flag_richardson) then
...
! $OMP END PARALLEL DO
       ...
       time_m = rnode(timemult,lstart(lcheck))
       write(outunit,101) lcheck,time_m,maxFlag  
 101   format(" level ",i3," time ",e12.5, " max flagged quantity ",
     .        e12.5)
