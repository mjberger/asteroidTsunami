...
c     # need to allocate for dynamic memory:
      ...
      read(rstunit) rnode(:,1:maxgrOld),node(:,1:maxgrOld),
     1       lstart,newstl,listsp,tl,
...
c
c     if enlarged nodal space need to thread it and add to old list
c     since above lists are built on the fly they dont need this 
c
      if (maxgr .gt. maxgrOld) then
         call rethread(maxgrOld)
      endif
...
c ---------------------------------------------------------
c
      subroutine rethread(maxgrOld)

      use amr_module
      implicit double precision (a-h,o-z)
c
c     add from maxgrOld to maxgr to end of nodal list
c     first thread new stuff, as in stst1
c
      do i = maxgrOld+1, maxgr
        node(nextfree,i) = i+1
      end do
      node(nextfree, maxgr) = ndfree
      ndfree = maxgrOld+1
c
      return
      end
