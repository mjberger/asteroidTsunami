...
!     by indices from mbestg1(mptr) to mbestg2(mptr)
      ...
      use storm_module, only: ambient_pressure,pressure_index
      ...
      real(kind=8) :: h(4),drytol2,topo,eta,press
      integer :: icell,jcell
...
!       write(6,*) '+++ interploting for gauge ', ii
     ...
     ! velocities are zeroed out which can then lead to increase in h again.
        ...
        if ((h(1) < drytol2) .or.  &
            ...
            press = aux(pressure_index,icell,jcell)
        else
            ...
            press = (1.d0 - xoff) * (1.d0 - yoff)                        &
                       * aux(pressure_index,iindex,jindex)               &
                + xoff*(1.d0-yoff)*aux(pressure_index,iindex+1,jindex)   &
                + (1.d0-xoff)*yoff*aux(pressure_index,iindex,jindex+1)   &
                + xoff*yoff*aux(pressure_index,iindex+1,jindex+1)
        endif
...
!$OMP CRITICAL (gaugeio)
        write(OUTGAUGEUNIT,100) igauge(ii),level,tgrid, &
                    (var(j),j=1,3),eta,                 &
                    (press-ambient_pressure)/ambient_pressure
