...
!
    ...
    ! Locals
    integer :: ii,jj,m, iint,jint,mptr
    real(kind=8) :: x,y,xm,ym,xp,yp,topo_integral,timeAll
    ...
    if (pressure_forcing) then
        !for variable pressure field uncomment next line
        !aux(pressure_index, :, :) = ambient_pressure  
        !for constant pressure field in time can set here, so uncomment next line
        timeAll = 0.d0
        mptr = -1   ! not used in this example, so make it something that will bomb is indexed
        call set_pressure_field(maux,mbc,mx,my,xlow,ylow,dx,dy,timeAll,aux,mptr)
    endif
