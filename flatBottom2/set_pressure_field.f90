          ...
          if (simpson) then
            ...
            do jloc = 1, 3
               ...
               do iloc = 1, 3
                  ...
                  overPressure = computedOverPressure(dist_in_m,time)       !  this version returns dp
            ...
            xuse = x
            dist_in_m = abs(xuse)
            overPressure = computedOverPressure(dist_in_m,time) 
...
double precision function  computedOverPressure(dist_in_m,time)
...
!  # use model of overpressure (based on MJA calcs) as a function of x,y,time
   ...
   ! Arguments
   real(kind=8), intent(in) :: dist_in_m, time
