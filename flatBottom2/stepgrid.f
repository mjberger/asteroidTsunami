...
c      dimension work(mwork)
      ...
      logical :: dump = .false.
c
      heightdif = -1.d0  ! initialized negative so at least will be set to 0 
      veldif = -1.d0 
...
c::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
     ...
     &             xlowmbc,ylowmbc,dx,dy,time,dt,maux,aux,mptr)
...
c       # update q
        ...
        dtdy = dt/dy
         if (mcapa.eq.0) then
          ...
          heightold = q(1,i,j)
          velold = sqrt((q(2,i,j)/q(1,i,j))**2+(q(3,i,j)/q(1,i,j))**2)
          do 49 m=1,nvar
c
c            # no capa array.  Standard flux differencing:
 ...
 49       continue
          if (abs(q(1,i,j)-heightold) .gt. heightdif) then
             heightdif = abs(q(1,i,j)-heightold)
             xmax = xlow + i*dx
             ymax = ylow + j*dy
          endif
          velnew = sqrt((q(2,i,j)/q(1,i,j))**2+(q(3,i,j)/q(1,i,j))**2)
          if (abs(velnew-velold) .gt. veldif) then
             veldif = abs(velnew-velold)
             vxmax = xlow + i*dx
             vymax = ylow + j*dy 
          endif
 50       continue
...
c        # with source term:   use Godunov splitting
     ...
     &             q,maux,aux,time,dt,veldif,vxmax,vymax)
         endif
