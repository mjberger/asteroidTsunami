...
c
      ...
      real*8 maxGradP2, maxGradP_thisGrid
...
!$OMP&            PRIVATE(locuse,maxGradP_thisGrid),
...
c no longer getting locbig, using "real" solution array in locnew
     ...
     &                        goodpt,badpt,maxGradP_thisGrid)

             maxGradP2 = max(maxGradP2, maxGradP_thisGrid)
             endif     
c     
         if (flag_richardson) then
...
! $OMP END PARALLEL DO
       ...
       mgrid = lstart(lcheck)
       time_m = rnode(timemult,mgrid)
       !write(outunit,101) mgrid,time_m,lcheck,maxGradP2
 101   format(" grid ",i4," time ",e12.5,"level ",i3,
     .        " max gradP sq ",e12.5)
