  ...
  601 format('AMRCLAW: Frame ',i4,
     &       ' output files done at time t = ', d12.6,/)
      ...
      if (output_format == 3) then
       ...
       call makeHovmoller(time,xstHov,ystHov,xendHov,yendHov,
     .                    nvar,naux,npts)
...
c     open for appending, need complete time history
     ...
     .         position='rewind',form='formatted')
         ...
         open(unit=ihovUnit,file='hovSlice.data',status='old',
     .         position='append',form='formatted')
      endif


c     make diagram using finest resolution
      hxfine = hxposs(mxnest)
      hyfine = hyposs(mxnest)
...
c     overwritten with finer grids if exists
 ...
 20      if (mptr .eq. 0) go to 30      
         ...
         if (xbegin .le. xlast .and. ybegin .le. ylast) then ! transect intersect this grid
            ...
            iptx = (xbegin-xstHov)/hxfine + 1
            ipty = (ybegin-ystHov)/hyfine + 1
            if (xstHov .eq. xendHov) then  ! line is vertical
              ipt = ipty
              iptend = (ylast - ystHov)/hyposs(mxnest)
              dyhov = hyposs(mxnest)
            else
              ipt = iptx
              iptend = (xlast - xstHov)/hxposs(mxnest)
              dxhov = hxposs(mxnest)
            endif
...
c            compute i,j index for source grid, interp val insert into hovLine
             ix = (xhov-xlo)/hxposs(level) + nghost  ! compute index into grid for interp
             jy = (yhov-ylo)/hyposs(level) + nghost  ! grid has ghost cells
